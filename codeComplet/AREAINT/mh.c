...
void StandardMH::birth( double t, Pattern& D )
{
   ...
   double area = prior->getArea(),w = whran.uniform();
...
void StandardMH::death( double t, Pattern& D )
{
   if( D.getSize() > 0 ) {
      ...
      double ratio= (D.getSize() + 1) / (area * prior->cond_intens( D, e));
...
AdaptedMH :: AdaptedMH( Prior *p, double cpb, double cpd, double cpp,
      double cps,double cpba,int ctime ) :
      Sampler(p)
{
  setTime(ctime);
  pb=cpb;
...
void AdaptedMH::birth( double t, Pattern& D )
{
   ...
   Pattern A;
   double na=(double)prior->countEventsToAdapt(D,A); 

   if(u<pba)
   	{
        if(na>0.0)
          {
          int pos=prior->selectEventToAdapt(A);
